...
def insertar_trazabilidad(datos: Dict[str, Any]) -> bool:
    """Inserta datos de trazabilidad en la base de datos con reconexión automática"""
    if not datos:
        return False
    ...
    try:
        ...
        try:
            ...
            if conn:
                cursor = conn.cursor()
                cursor.execute(sql, values)
                log.info(f"Reintentado insertado en trazabilidad: {datos}")
                return True
        except Exception as retry_e:
            log.error(f"Error en reintento insertando trazabilidad: {retry_e}")
        return False
    finally:
        try:
            cursor.close()
        ...
        while not _stop:
            ...
            if espera > 0:
                ...
                while time.time() - start_wait < espera and not _stop:
                    await asyncio.sleep(0.1)  # Verificar cada 100ms para mayor responsividad
